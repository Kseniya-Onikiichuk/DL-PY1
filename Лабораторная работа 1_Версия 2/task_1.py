...
class Manicure:
    ...
    def nail_colour_check(self) -> bool:
        """
        Функция, которая проверяет, накрашен ли ноготь
        True - ноготь накрашен, False - ноготь чист

        :return: Присутствует ли на ногте цвет

         Примеры:
        >>> manic_01 = Manicure(5, "green")
        >>> manic_01.nail_colour_check()
        """
        ...
...
class Flower:
    ...
    def add_flowers(self, new_buds: int) -> None:
        """
        Функция, которая добавляет к исходному количеству цветков некоторое количество новых

        :param new_buds: Количество новых цветков (бутонов)
        :raise TypeError: Если количество новых цветков задано не целым числом

        Примеры:
        >>> flower_01 = Flower(30, 4)
        >>> flower_01.add_flowers(3)
        """
        ...
